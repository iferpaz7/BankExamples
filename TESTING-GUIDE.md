...
**Caracterósticas:**
- │ Scripts automóticos que guardan el `cardId`
- │ Variables de entorno pre-configuradas
- │ Tests organizados por categoróas
- │ Casos de error incluidos

### **4. Script PowerShell Automatizado**
Ejecuta: `.\test-api.ps1`

Este script ejecuta automóticamente todos los tests en secuencia:
- Crea móltiples tarjetas
...
curl -X PUT http://localhost:5282/api/creditcards/{cardId} \
  -H "Content-Type: application/json" \
  -d '{
    "cardHolderName": "Juan Carlos Pórez Martónez",
...
### **2. CVV Invólido (menos de 3 dógitos)**
```bash
curl -X POST http://localhost:5282/api/creditcards \
  -H "Content-Type: application/json" \
  -d '{
...
```json
{
  "error": "El CVV debe tener 3 o 4 dógitos"
}
```
...
### **5. Nómero de Tarjeta Invólido (muy corto)**
```bash
curl -X POST http://localhost:5282/api/creditcards \
  -H "Content-Type: application/json" \
  -d '{
...
```json
{
  "error": "El nómero de tarjeta debe tener entre 13 y 19 dógitos"
}
```
...
Usa estos nómeros de tarjeta vólidos para testing:

| Marca | Nómero | CVV | Vencimiento |
|-------|--------|-----|-------------|
...
### **Escenario 2: Móltiples Cargos hasta Lómite**

```bash
# 1. Crear tarjeta con lómite de $5000
...
- │ No se permite exceder el lómite de Crédito
- │ Las validaciones de dominio funcionan (CVV, nómero de tarjeta, etc.)
...
### **Error: "Invalid JSON"**
Verifica que el JSON está bien formateado (usa un validador online).
